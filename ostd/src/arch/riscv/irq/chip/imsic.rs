...
impl Imsic {
    ...
    pub(super) fn from_fdt(fdt: &Fdt<'_>, io_mem_builder: &IoMemAllocatorBuilder) -> Option<Self> {
        ...
        let regions = node
            ...
            .map(|region| {
                ...
                io_mem_builder.reserve(start..start + size, crate::mm::CachePolicy::Uncacheable)
            })
            .collect::<Vec<_>>()
            .into_boxed_slice();
    ...
    pub(super) fn enable(&self, interrupt_id: u8) -> bool {
        let interrupt_id = interrupt_id as u32;
        if interrupt_id == 0 || interrupt_id > self.num_ids {
            return false;
        }

        let register_index = interrupt_id as usize / 64;
        let bit_index = interrupt_id as usize % 64;
    ...
    pub(super) fn disable(&self, interrupt_id: u8) {
        let interrupt_id = interrupt_id as u32;
        if interrupt_id == 0 || interrupt_id > self.num_ids {
            return;
        }
    ...
    pub(super) fn claim(&self) -> Option<u8> {
        let value: usize;
        ...
        u8::try_from(interrupt_id).ok().filter(|id| *id != 0)
    }
