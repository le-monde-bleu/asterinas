...
const MIN_REGISTER_REGION_SIZE: usize = REG_ICVEC + size_of::<u64>();

pub(super) const FCTL_BE: u32 = 1 << 0;
pub(super) const FCTL_WSI: u32 = 1 << 1;
pub(super) const FCTL_GXL: u32 = 1 << 2;
pub(super) const ICVEC_FIV_SHIFT: u32 = 4;
pub(super) const ICVEC_FIV_MASK: u64 = 0x0f << ICVEC_FIV_SHIFT;
pub(super) const DDTP_BUSY: u64 = 1 << 4;
...
impl Capability {
    ...
    pub const fn interrupt_generation_support(&self) -> InterruptGenerationSupport {
        match (self.0 >> 28) & 0x03 {
            0 => InterruptGenerationSupport::Msi,
            1 => InterruptGenerationSupport::Wired,
            2 => InterruptGenerationSupport::Both,
            _ => InterruptGenerationSupport::Reserved,
        }
    }
...
/// Interrupt generation methods reported by `capabilities.IGS`.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) enum InterruptGenerationSupport {
    Msi,
    Wired,
    Both,
    Reserved,
...
// IOMMU-originated MSI delivery uses the optional configuration table at
// 0x300–0x3ff. This driver currently selects wired delivery when available.
pub(super) struct IommuRegisters {
    ...
    pub pqcsr: VolatileRef<'static, u32, ReadWrite>,
...
impl IommuRegisters {
    ...
    /// # Safety
    ///
    /// `base_register_vaddr` must be 8-byte aligned and point to an exclusively
    /// owned, mapped IOMMU register region of at least
    /// [`MIN_REGISTER_REGION_SIZE`] bytes.
    unsafe fn new(base_register_vaddr: NonNull<u8>) -> Self {
        // SAFETY: The caller guarantees alignment, sufficient mapped length,
        // and exclusive ownership for every register reference constructed below.
        unsafe {
            ...
            Self {
                ...
                pqcsr: VolatileRef::new(base_register_vaddr.add(REG_PQCSR).cast::<u32>()),
...
pub(super) fn init(io_mem_builder: &IoMemAllocatorBuilder) -> Result<(), IommuError> {
    ...
    if base_address == 0
        || !base_address.is_multiple_of(align_of::<u64>())
        || region_size < MIN_REGISTER_REGION_SIZE
    {
        return Err(IommuError::InvalidRegisterRegion);
    }

    let region_end = base_address
        .checked_add(region_size)
        .ok_or(IommuError::InvalidRegisterRegion)?;
    ...
    // SAFETY: The validated device-tree region is 8-byte aligned, covers every
    // register accessed by `IommuRegisters`, and was removed from the general
    // MMIO allocator to provide exclusive ownership.
